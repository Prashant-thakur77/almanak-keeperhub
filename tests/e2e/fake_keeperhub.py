...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        if self.path == "/api/user":
            self._send(200, {"id": "fake-user", "email": "fake@wallet.keeperhub.com", "walletAddress": self.state.account.address})
            return
        if self.path == "/api/chains":
            self._send(200, [{"chainId": self.state.chain_id, "name": "fake-fork", "isEnabled": True, "isTestnet": False}])
            return
    ...
    def _simulate(self, to: str, data: bytes, value_wei: int, refusal: str | None) -> None:
        ...
        if refusal:
            self._send(400, {"success": False, "status": "simulated", "from": sender, "to": to, "failureKind": "validation", "wouldRevert": True, "revertReason": refusal, "error": refusal})
            return
        ...
        try:
            ...
            reason = str(exc)
            self._send(400, {"success": False, "status": "simulated", "from": sender, "to": to, "value": str(value_wei), "failureKind": "revert", "wouldRevert": True, "revertReason": reason, "error": reason})
            return
        self._send(200, {"success": True, "status": "simulated", "from": sender, "to": to, "value": str(value_wei), "gasEstimate": str(gas), "simulatedReturnValue": None, "wouldRevert": False})

    def _execute(self, key: str, body: dict[str, Any], to: str, data: bytes, value_wei: int, refusal: str | None) -> None:
        digest = hashlib.sha256(json.dumps(body, sort_keys=True).encode()).hexdigest()
        with self.state.lock:
            seen = self.state.idempotency.get(key)
            if seen and seen[0] != digest:
                self._send(409, {"error": "Idempotency-Key reused with a different body", "code": "idempotency_conflict", "retryable": False, "originalExecutionId": seen[1]})
                return
            if seen:
                replay = dict(self.state.executions[seen[1]])
                replay["idempotentReplay"] = True
                self._send(202, {k: replay.get(k) for k in ("executionId", "status", "transactionHash", "transactionLink", "error", "idempotentReplay")})
                return
            execution_id = uuid.uuid4().hex[:21]
            self.state.idempotency[key] = (digest, execution_id)
            if refusal:
                execution = {"executionId": execution_id, "status": "failed", "type": "contract-call", "network": str(self.state.chain_id), "transactionHash": None, "transactionLink": None, "receipts": [], "error": refusal, "sponsored": False}
                self.state.executions[execution_id] = execution
                self._send(202, {"executionId": execution_id, "status": "failed", "error": refusal})
                return
            execution = {"executionId": execution_id, "status": "running", "type": "contract-call", "network": str(self.state.chain_id), "transactionHash": None, "transactionLink": None, "receipts": [], "error": None, "sponsored": False}
            self.state.executions[execution_id] = execution
        web3 = self.state.web3
        sender = self.state.account.address
        tx = {"from": sender, "to": Web3.to_checksum_address(to), "data": data, "value": value_wei, "chainId": self.state.chain_id, "nonce": web3.eth.get_transaction_count(sender, "pending")}
        try:
            ...
            return
        execution.update(transactionHash=tx_hash, transactionLink=f"https://fake.explorer/tx/{tx_hash}", status="unconfirmed")
        ...
        while time.time() < deadline and receipt is None:
            try:
                ...
                time.sleep(0.2)
        if receipt is None:
            self._send(202, {"executionId": execution_id, "status": "unconfirmed", "transactionHash": tx_hash, "transactionLink": execution["transactionLink"]})
            return
        ok = receipt["status"] == 1
        execution.update(
            status="completed" if ok else "failed",
            error=None if ok else "execution reverted",
            receipts=[{"hash": tx_hash, "chainId": self.state.chain_id, "verified": True, "receiptStatus": "success" if ok else "reverted", "blockNumber": receipt["blockNumber"], "gasUsed": str(receipt["gasUsed"])}],
            gasUsedWei=str(receipt["gasUsed"] * receipt["effectiveGasPrice"]),
        )
        payload = {"executionId": execution_id, "status": execution["status"], "transactionHash": tx_hash, "transactionLink": execution["transactionLink"]}
        if not ok:
