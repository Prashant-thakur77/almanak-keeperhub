...
|---|---|---|---|---|
| Integration depth | 9 | KeeperHub sits inside Almanak's own execution interfaces (`Signer`, `Submitter`, `Simulator`) and gateway plugin points; unmodified strategies and Almanak's agent CLI (`ax`) both execute through it | n8n (3rd) shipped a node inside a live product with users and got it verified by n8n; three public Almanak entries integrate one level up (intent translators) | Same shape as n8n: inside the partner's own extension mechanism, not beside it |
...
- Surfaces breadth: REST direct execution and the audit trail only. That is a deliberate choice, stated as such.
